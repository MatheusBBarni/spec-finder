...
describe("listActivePackets", () => {
  test("classifies remaining, early-stage, blocked, invalid, and all-complete packets", async () => {
    ...
    await writeFile(join(blockedCheckpointDir, "task_01.md"), completedTask(1, "Blocked delivery", blockedCheckpoint()))

    const blockedHandoff = await packetDir(tasks, "blocked-handoff")
    await writeFile(join(blockedHandoff, "task_01.md"), taskMarkdown(1, "Handoff error", "handoff:\n  phase: report\n  error: report process exited\n"))

    const handoffNoError = await packetDir(tasks, "handoff-remaining")
    await writeFile(join(handoffNoError, "task_01.md"), taskMarkdown(1, "Handoff pending", "handoff:\n  phase: report\n"))
    ...
    await writeFile(join(invalidDeps, "task_01.md"), taskMarkdown(1, "Unknown dep").replace("dependencies: []", "dependencies: [task_99]"))
    ...
    await writeFile(join(root, ".spec-finder", "tasks_done", "archived", "task_01.md"), taskMarkdown(1, "Archived"))
    ...
    expect(bySlug(result.rows, "remaining-work")).toEqual({ slug: "remaining-work", kind: "remaining", completed: 1, total: 2 })
    expect(bySlug(result.rows, "early-stage")).toEqual({ slug: "early-stage", kind: "early-stage", completed: 0, total: 0 })
    expect(bySlug(result.rows, "blocked-checkpoint")).toEqual({ slug: "blocked-checkpoint", kind: "blocked", completed: 0, total: 1 })
    expect(bySlug(result.rows, "blocked-handoff")).toEqual({ slug: "blocked-handoff", kind: "blocked", completed: 0, total: 1 })
    expect(bySlug(result.rows, "handoff-remaining")).toEqual({ slug: "handoff-remaining", kind: "remaining", completed: 0, total: 1 })
    expect(bySlug(result.rows, "all-complete")).toEqual({ slug: "all-complete", kind: "remaining", completed: 2, total: 2 })
    ...
    expect(bySlug(result.rows, "Invalid_Slug")).toEqual({ slug: "Invalid_Slug", kind: "invalid", completed: 0, total: 0, detail: "invalid slug" })
  })
...
describe("inspectPacket", () => {
  test("includes pending ids and blocked checkpoint reason", async () => {
    ...
    await writeFile(join(directory, "task_01.md"), completedTask(1, "Blocked delivery", blockedCheckpoint()))
    ...
    expect(result.blockers).toEqual([{ taskId: "task_01", kind: "checkpoint", message: "gitignore blocked delivery" }])
  ...
  test("treats report handoff without error as remaining work", async () => {
    ...
    await writeFile(join(directory, "task_01.md"), taskMarkdown(1, "Handoff pending", "handoff:\n  phase: report\n"))
  ...
  test("maps loop absent, none, terminal, and invalid without failing the packet", async () => {
    ...
    if (invalid.loop.state === "invalid") expect(invalid.loop.message).toContain("invalid JSON")
  })
  ...
  test("returns missing and invalid_slug failures", async () => {
    ...
    expect(await inspectPacket(root, "no-such")).toEqual({ ok: false, code: "missing", message: "packet is missing: no-such" })
    expect(await inspectPacket(root, "Invalid_Slug")).toEqual({ ok: false, code: "invalid_slug", message: "invalid task slug: Invalid_Slug" })
  })
