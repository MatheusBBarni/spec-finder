...
describe("setup", () => {
  test("creates config, task root, and skills for selected agents", async () => {
    ...
    await access(join(root, ".agents", "skills", "sf-batch-tasks", "SKILL.md"))
    await access(join(root, ".cursor", "skills", "sf-task-report", "SKILL.md"))
    await access(join(root, ".cursor", "skills", "sf-archive-tasks", "scripts", "scan-tasks.sh"))
  ...
  test("uses Codex as the canonical copied target and links other selected providers", async () => {
    ...
    expect(first.copied).toHaveLength(9)
    expect(first.linked).toHaveLength(18)
